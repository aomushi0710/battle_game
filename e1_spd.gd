...
@onready var buff_icon = $/root/Node2D/enemies/enemy1/e1_icon/バフアイコン
@onready var buff_turn = $/root/Node2D/enemies/enemy1/e1_icon/buff_turn
@onready var debuff_icon = $/root/Node2D/enemies/enemy1/e1_icon/デバフアイコン
@onready var debuff_turn = $/root/Node2D/enemies/enemy1/e1_icon/debuff_turn
...
func _process(delta): # ゲージが溜まるまで自動で増加、溜まると停止
	...
	if $".".value < Global.spd_gauge: # SPDの値ずつ毎秒増加していき、500まで到達するとコマンドリストを表示
		...
		set_process(false)
		for button in $"../e1_command".get_children():
