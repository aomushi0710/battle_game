...
@onready var buff_icon = $/root/Node2D/buttle/player2/p2_icon/バフアイコン
@onready var buff_turn = $/root/Node2D/buttle/player2/p2_icon/buff_turn
@onready var debuff_icon = $/root/Node2D/buttle/player2/p2_icon/デバフアイコン
@onready var debuff_turn = $/root/Node2D/buttle/player2/p2_icon/debuff_turn
...
func _process(delta): # ゲージが溜まるまで自動で増加、溜まると停止
	...
	if $".".value < Global.spd_gauge: # SPDの値ずつ毎秒増加していき、500まで到達するとコマンドリストを表示
		...
		set_process(false)
		for button in $"../p2_command".get_children():
...
func _on_コマンド_spd(): # ターン経過処理
	...
	buttle_script.buff_icon(true,index)
