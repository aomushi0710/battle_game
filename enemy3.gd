...
var index := 2
var monster_dict: Dictionary = Global.enemy_deck.monster_dict[index]
var monster: Monster = Global.enemy_deck.monster[index]
var action: Array = Global.enemy_deck.action[index] # Array[Action]
var evolution: Array = Global.enemy_deck.evolution[index] # Array[Action]
var middle_evolution: Array = Global.enemy_deck.middle_evolution[index] # Array[Action]
...
func _evolution():
	...
	Global.enemy_deck.monster[index] = evolution_monster.duplicate() # 進化
	...
	for i: int in len(Global.enemy_deck.action[index]): # 進化技を置き換え
		if Global.enemy_deck.action[index][i].id == id:
			match id:
				10001:
					Global.enemy_deck.action[index][i] = \
					Global.enemy_deck.middle_evolution[index][act]
				10002:
					Global.enemy_deck.action[index][i] = \
					Global.enemy_deck.evolution[index][act]
				_:
					print("ERROR:不明なID")
			act += 1
	
	$"/root/Node2D/log_window/log".text += "[color=red]相手の " + name + \
