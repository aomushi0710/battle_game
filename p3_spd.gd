...
@onready var buff_icon = $/root/Node2D/buttle/player3/p3_icon/バフアイコン
@onready var buff_turn = $/root/Node2D/buttle/player3/p3_icon/buff_turn
@onready var debuff_icon = $/root/Node2D/buttle/player3/p3_icon/デバフアイコン
@onready var debuff_turn = $/root/Node2D/buttle/player3/p3_icon/debuff_turn
...
func _process(delta): # ゲージが溜まるまで自動で増加、溜まると停止
	if $".".value < Global.spd_gauge: # SPDの値ずつ毎秒増加していき、500まで到達するとコマンドリストを表示
		...
		set_process(false)
		for button in $"../p3_command".get_children():
