...
var action = Global.enemy_deck.action[index] # Array[Action]
var chance = Global.enemy_deck.chance[index] # Array[int]
var act_range = [] # 乱数幅格納
...
func _on_p_1_spd_command(): # 一旦、actionsにappendし、後でまとめてボタンインスタンスを
	...
	select_command(randi() % 4) # ランダムに技送信
...
func element_icon(button: Button, element_list: Array) -> void:
	if len(element_list) == 1: # 属性が1つだけだった場合
		...
		button.get_child(0).self_modulate = Color(0.5, 0.5, 0.5, 1) # この行は敵のみ
...
func select_command(x :int): # x 押されたボタンの技のindex
	...
	command.emit(actions.pop_at(x), monster, false, index)
	spd.emit()
