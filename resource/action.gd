...
## 技を持つモンスターのデータ[param monster]とその現在の形態[param form]を引数として、
##その技を利用可能になる形態に達していなければ、
##対応する形態へ進化させる技として変換してからその技を返す関数
func evolution_check(monster: MonsterData, form: Global.Form) -> Action:
	if unlock_form > form:
		...
		evolution_action.data.mp = monster.evolution_forms[unlock_form].cost
